...
public class Untils {
    ...
    public static Status inputProjectStatus(Scanner sc) {
        Status status = null;
        while (status == null) {
            try {
                System.out.println("Nhập trạng thái (CHUA_BAT_DAU, DANG_THUC_HIEN, DA_HOAN_THANH, TAM_DUNG, DA_HUY):");
                String statusInput = sc.nextLine().toUpperCase();
                status = Status.valueOf(statusInput);  // Chuyển chuỗi nhập thành enum Status
